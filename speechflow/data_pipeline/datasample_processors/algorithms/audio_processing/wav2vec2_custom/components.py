...
class ConvLayerBlock(Module):
    ...
    def __init__(
        ...
        self.pad = math.floor(self.kernel_size / 2)
    ...
    def forward(
        ...
        """
        x = torch.nn.functional.pad(x, (self.pad, 0, 0, 0, 0, 0), "constant", 0)
        x = self.conv(x)
        ...
        if length is not None:
            length = torch.div(length, self.stride, rounding_mode="floor")
        return x, length
...
class FeatureProjection(Module):
    """Layer that connects FeatureExtractor and Encoder Projects features to encoder
    dimension.
...
class Transformer(Module):
    ...
    def forward(
        ...
        all_result = []
        for ids, layer in enumerate(self.layers):
...
class Encoder(Module):
    ...
    def create_chunk_mask(self, att_mask, max_audio_length, batch_size):
        ...
        if chunk_size is None:
            ...
            # chunk_idx = torch.div(chunk_idx, chunk_size//2, rounding_mode="trunc")
            # diff_chunks_half = chunk_idx.unsqueeze(1) - chunk_idx.unsqueeze(0)
            # chunked_limited_mask_2 = torch.logical_and(chunked_limited_mask, torch.le(diff_chunks_half, 2))
            chunked_limited_mask = chunked_limited_mask[None, None, :, :].expand(
                batch_size, 1, max_audio_length, max_audio_length
            )
            att_mask = torch.logical_not(
                torch.logical_and(torch.logical_not(att_mask), chunked_limited_mask)
            )
        return att_mask
    ...
    def _preprocess(
        ...
        if lengths is not None:
            ...
            x[mask] = 0.0
            mask = mask[:, None, None, :].expand(batch_size, 1, max_len, max_len)
            if self.training:
                mask = self.create_chunk_mask(mask, max_len, batch_size)
            mask = -10000.0 * mask.to(dtype=features.dtype)
    ...
    def forward(
        ...
        x, all_result = self.transformer(x, attention_mask=mask)
        return x, all_result
