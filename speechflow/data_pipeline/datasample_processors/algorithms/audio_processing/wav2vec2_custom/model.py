...
class Wav2Vec2Model(Module):
    ...
    def extract_features(
        ...
        """Extract feature vectors from raw waveforms This returns the list of outputs
        from the intermediate layers of transformer block in encoder.
    ...
    def forward(
        ...
        """
        x, lengths = self.feature_extractor(waveforms, lengths)
        x, all_result = self.encoder(x, lengths)
        if self.aux is not None:
            x = self.aux(x)
        if return_all_feature:
            return x, lengths, all_result
        else:
            return x, lengths, []
