...
class MultiPeriodDiscriminator(nn.Module):
    ...
    Additionally, it allows incorporating conditional information with a learned
    embeddings table.
...
class MultiBandDiscriminator(nn.Module):
    def __init__(
        self,
        mbd_fft_sizes: tp.Tuple[int] = (2048, 1024, 512),
    ):
        """Multi-band multi-scale STFT discriminator, with the architecture based on
...
class DiscriminatorB(nn.Module):
    ...
    def spectrogram(self, x: torch.Tensor) -> tp.List[torch.Tensor]:
        # Remove DC offset
        x = x - x.mean(dim=-1, keepdims=True)
        # Peak normalize the volume of input audio
        x = 0.8 * x / (x.abs().max(dim=-1, keepdim=True)[0] + 1e-9)
...
class MultiScaleSubbandCQTDiscriminator(nn.Module):
    def __init__(self, sample_rate: int):
        ...
        self.cfg["cqtd_filters"] = self.cfg.get("cqtd_filters", 128)
...
class DiscriminatorCQT(nn.Module):
    def __init__(
        self, cfg: tp.Dict, hop_length: int, n_octaves: int, bins_per_octave: int
    ):
        super().__init__()
        self.cfg = cfg
