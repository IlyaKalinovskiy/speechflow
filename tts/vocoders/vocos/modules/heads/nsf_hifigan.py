...
__all__ = ["NSFHiFiGANHead", "NSFHiFiGANHeadParams"]


class NSFHiFiGANHeadParams(BaseTorchModelParams):
    input_dim: int = 512
    inner_dim: int = 1024
...
class NSFHiFiGANHead(WaveformGenerator):
    params: NSFHiFiGANHeadParams

    def __init__(self, params: NSFHiFiGANHeadParams):
...
class Generator(torch.nn.Module):
    def __init__(
        ...
        self.f0_ups = torch.nn.Upsample(scale_factor=np.prod(upsample_rates))
    ...
    def forward(self, x, s, f0):
        f0 = self.f0_ups(f0[:, None]).transpose(1, 2)  # bs,n,t
