...
class VALLF(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._is_random_position = is_random_position
        ...
        self.ar_text_position = SinePositionalEmbedding(
            ...
            scale=False,
            alpha=True,
            is_random=self._is_random_position,
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if train_stage in [0, 2]:
            ...
            y_len = y_lens.max()
            targets = codes[..., nar_stage] + self._num_audio_tokens * y_mask_int
...
class VALLE(VALLF):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            ...
            is_random_position=is_random_position,
            **kwargs,
        )
    ...
    def forward(
        ...
        x: torch.Tensor,
        x_lens: torch.Tensor,
        # y: tp.Union[torch.Tensor, PromptedFeatures],
        # y_lens: tp.Union[torch.Tensor, PromptedFeatures],
        y: torch.Tensor,
        y_lens: torch.Tensor,
        y_prompts_codes: tp.Optional[torch.Tensor] = None,
        prompts_len: tp.Optional[torch.Tensor] = None,
        reduction: str = "sum",
        train_stage: int = 0,
        **kwargs,
    ) -> tp.Tuple[torch.Tensor, tp.Union[torch.Tensor, None]]:
        ...
        if train_stage in [0, 1]:
            x = self.ar_text_embedding(text)
            x = self.ar_text_prenet(x)
            x, pos_begin = self.ar_text_position(x)
            ...
            y_emb = self.ar_audio_embedding(y)
            y_emb = self.ar_audio_prenet(y_emb)
            y_pos, pos_begin = self.ar_audio_position(y_emb, pos_begin)

            xy_pos = torch.concat([x, y_pos], dim=1)

            # if self._total_max_len > 0:
            #     xy_pos = xy_pos[:, :self._total_max_len, :]
            #     targets = targets[:, :self._total_max_len - x_len]
            #     xy_attn_mask = xy_attn_mask[..., :self._total_max_len, :self._total_max_len]
    ...
    def inference(
        ...
        assert torch.all(x_lens > 0)
        print("VALLE concat 1 inference")
