...
class RotaryPositionalEmbeddings(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        """
        * `x` is the Tensor at the head of a key or a query with shape `[seq_len, batch_size, n_heads_enc, d]`
        """
        # Cache $\cos$ and $\sin$ values
...
class TextEncoder(nn.Module):
    def __init__(
        ...
        self.encoder = Encoder(
            encoder_params.n_channels + (speaker_emb_dim if n_spks > 1 else 0),
            encoder_params.filter_channels,
            encoder_params.n_heads_enc,
        ...
        self.proj_m = torch.nn.Conv1d(
            self.n_channels + (speaker_emb_dim if n_spks > 1 else 0), self.n_feats, 1
        )
        self.proj_w = DurationPredictor(
            self.n_channels + (speaker_emb_dim if n_spks > 1 else 0),
