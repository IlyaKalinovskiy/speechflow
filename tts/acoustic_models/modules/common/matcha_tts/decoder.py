...
class Block1D(torch.nn.Module):
    ...
    def forward(self, x, mask):
        output = self.block(apply_mask(x, mask))
...
class ResnetBlock1D(torch.nn.Module):
    ...
    def forward(self, x, mask, time_emb):
        ...
        output = h + self.res_conv(apply_mask(x, mask))
        return output
...
class Decoder(nn.Module):
    ...
    def get_block(
        ...
        if block_type == "conformer":
            ...
            block = BaseTransformerBlock(
                ...
                time_embeds_ada_norm_dim=time_dim,
                cond_embeds_ada_norm_dim=cond_dim,
                norm_type=norm_type,
            )
        else:
    ...
    def forward(self, x, mask, mu, t, spks=None, cond=None):
        ...
        for resnet, transformer_blocks, downsample in self.down_blocks:
            ...
            x = downsample(apply_mask(x, mask_down))
        ...
        for resnet, transformer_blocks, upsample in self.up_blocks:
            mask_up = masks.pop()
            x = resnet(pack([x, hiddens.pop()], "b * t")[0], mask_up, t_mlp)
            x = rearrange(x, "b c t -> b t c")
            ...
            x = upsample(apply_mask(x, mask_up))

        x = self.final_block(x, mask_up)
        output = self.final_proj(apply_mask(x, mask_up))
