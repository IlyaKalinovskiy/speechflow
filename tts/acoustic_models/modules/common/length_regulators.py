...
class LengthRegulator(nn.Module):
    ...
    def regulate_length(self, x, durations, max_len):
        output = list()
        mel_len = list()
    ...
    def forward(
        ...
        output, mel_len = self.regulate_length(x, durations, max_len)
...
class SoftLengthRegulator(nn.Module):
    ...
    def _softmax_interpolation(
        ...
        with torch.no_grad():
            if max_len is None:
                max_len = get_lengths_from_durations(durations).max()
    ...
    def forward(
        ...
        # mask = get_mask_from_lengths2(get_lengths_from_durations(durations))
